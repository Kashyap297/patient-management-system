...
const CreatePrescriptionForm = ({ onFormUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(
        "Prescription created successfully and appointment marked as Completed"
      );
      navigate(`/doctor/prescription-tools/create`);
    } catch (error) {
      console.error(
        "Error creating prescription or updating appointment status:",
        error
      );
    }
  };
  ...
  return (
    <form
      ...
      className="flex flex-col gap-6 bg-white w-full max-w-4xl mx-auto"
    >
      ...
      {formValues.medicines.map((medicine, index) => (
        <div
          ...
          className={`grid grid-cols-[2fr_1fr_1fr_1fr_2fr_auto] gap-2 items-center mb-4 ${
            medicine.isEnabled ? "bg-white" : "bg-white"
          }`}
        >
          ...
          <div className="relative">
            <select
              ...
              onChange={(e) =>
                handleMedicineChange(index, "whenToTake", e.target.value)
              }
              className="w-full border border-gray-300 p-2 rounded-xl focus:outline-none"
              disabled={!medicine.isEnabled}
            >
